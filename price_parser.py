...
class KeyPriceParser:
    ...
    async def update_offer_price(self, offer_id, new_price, offer_type, variant_data):
        """Обновление цены оффера через PATCH запрос"""
        try:
            adjusted_price = round(new_price, 2)
            ...
            update_data = {
                ...
                "variant": {
                    ...
                    "status": variant_data.get("active", 'active'),
                    "visibility": variant_data.get("visibility", "all")
                }
            }

            # Добавляем inventory если есть
            if "inventory" in variant_data:
                update_data["variant"]["inventory"] = variant_data["inventory"]
            ...
            if result.get("success"):
                print(f"Цена обновлена: retail={adjusted_price}, business={adjusted_price}")
                return True
            else:
