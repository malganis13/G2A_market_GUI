...
class G2AAutomationGUI(ctk.CTk):
    ...
    def setup_settings_tab(self):
        ...
        scrollable = ctk.CTkScrollableFrame(frame, width=700, height=750)
        scrollable.pack(fill="both", expand=True, padx=10, pady=10)

        # G2A API Settings
        ctk.CTkLabel(scrollable, text="G2A API Настройки", font=("Arial", 20, "bold")).pack(pady=15)
        ...
        self.client_id_entry = ctk.CTkEntry(scrollable, textvariable=self.client_id_var, width=600, height=35)
        self.client_id_entry.pack(pady=5)

        ctk.CTkLabel(scrollable, text="G2A Client Secret:", font=("Arial", 12)).pack(pady=5)
        self.client_secret_entry = ctk.CTkEntry(scrollable, textvariable=self.client_secret_var, width=600, height=35,
                                                show="*")
        self.client_secret_entry.pack(pady=5)

        ctk.CTkLabel(scrollable, text="G2A Account Email:", font=("Arial", 12)).pack(pady=5)
        self.client_email_entry = ctk.CTkEntry(scrollable, textvariable=self.client_email_var, width=600, height=35)
        ...
        # Telegram Settings
        ctk.CTkLabel(scrollable, text="Telegram Уведомления", font=("Arial", 20, "bold")).pack(pady=15)
        ...
        self.telegram_token_entry = ctk.CTkEntry(scrollable, textvariable=self.telegram_token_var, width=600, height=35)
        self.telegram_token_entry.pack(pady=5)

        ctk.CTkLabel(scrollable, text="Chat ID:", font=("Arial", 12)).pack(pady=5)
        self.telegram_chat_entry = ctk.CTkEntry(scrollable, textvariable=self.telegram_chat_var, width=600, height=35)
        ...
        self.telegram_checkbox.pack(pady=15)
    ...
    def setup_auto_tab(self):
        ...
        scrollable = ctk.CTkScrollableFrame(left_frame, width=450, height=700)
        scrollable.pack(fill="both", expand=True, padx=5, pady=5)

        ctk.CTkLabel(scrollable, text="Настройки автоизменения", font=("Arial", 18, "bold")).pack(pady=15)
        ...
        ctk.CTkLabel(right_frame, text="Управление автоизменением", font=("Arial", 18, "bold")).pack(pady=15)
        ...
        # Лог автоизменений
        ctk.CTkLabel(right_frame, text="Лог событий:", font=("Arial", 14, "bold")).pack(pady=10)
        self.auto_log = ctk.CTkTextbox(right_frame, width=650, height=450, font=("Courier", 10))
    ...
    def setup_parsing_tab(self):
        ...
        ctk.CTkLabel(frame, text="Парсинг цен G2A", font=("Arial", 20, "bold")).pack(pady=20)
        ...
        self.progress_bar = ctk.CTkProgressBar(frame, variable=self.progress_var, width=700, height=25)
        self.progress_bar.pack(pady=20)
        self.progress_bar.set(0)

        self.log_text = ctk.CTkTextbox(frame, width=900, height=550, font=("Courier", 11))
    ...
    def setup_offers_tab(self):
        ...
        ctk.CTkLabel(left_frame, text="Список офферов", font=("Arial", 18, "bold")).pack(pady=10)
        ...
        ctk.CTkLabel(right_frame, text="Управление оффером", font=("Arial", 18, "bold")).pack(pady=15)

        # ✅ ИСПРАВЛЕНО: Добавлена прокрутка для кнопок
        scrollable_right = ctk.CTkScrollableFrame(right_frame, width=420, height=750)
        ...
        ctk.CTkLabel(scrollable_right, text="Точечные операции:", font=("Arial", 14, "bold")).pack(pady=10)
        ...
        ctk.CTkLabel(scrollable_right, text="─" * 35).pack(pady=15)

        ctk.CTkLabel(scrollable_right, text="Массовые операции:", font=("Arial", 14, "bold")).pack(pady=10)
    ...
    def setup_keys_tab(self):
        ...
        ctk.CTkLabel(frame, text="Управление ключами", font=("Arial", 20, "bold")).pack(pady=20)
        ...
        self.stats_scrollable = ctk.CTkScrollableFrame(frame, width=900, height=600)
    ...
    def setup_stats_tab(self):
        ...
        ctk.CTkLabel(frame, text="Статистика изменений цен", font=("Arial", 20, "bold")).pack(pady=20)
        ...
        self.stats_text = ctk.CTkTextbox(frame, width=950, height=650, font=("Courier", 11))
    ...
    def load_offers(self):
        """✅ ИСПРАВЛЕНО: Загрузка офферов без lambda ошибки"""
        ...
        def run():
            ...
            try:
                ...
                if result.get("success"):
                    ...
                    # ✅ ИСПРАВЛЕНО: Именованная функция вместо lambda
                    def show_error():
